...
max_holded_files: int = 10
request_time_offset: float = 1.0
files_created_to_delete: list[str] = [] # delete after reaching past limit = 10

args = sys.argv
if len(args) < 4:
    print(f"Missing params: {len(args)} given {4} required!\n")
    quit(1)

try:
    limit_requests = int(args[1])
    max_holded_files: int = int(args[2])
    request_time_offset: float = float(args[3])
except:
...
def randomize_command() -> tuple[list[str], list[int]]:
    ...
    match choice:
        ...
        case 'ts':
            ...
            out_file_name: str = 'output_'+client_files_id+'.txt'
...
while True:
    ...
    if 'info' == auto_message[0]:
        ...
        with open(os.path.join(cwd, 'client_requests_data', 'results', client_files_id+'.txt'), 'w') as file:
    ...
    if len(files_created_to_delete) > max_holded_files: 
        pass # delete all files in array of names TODO
    # print('command complete\n')

# print('autoclient quit')
server.close()